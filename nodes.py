...
inference_mode_list = ['预训练音色', '3s极速复刻', '跨语种复刻', '自然语言控制', '变声']
...
class CosyVoiceNode:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "tts_text": ("TEXT",),
                "prompt_wav": ("AUDIO",),
                "source_wav": ("AUDIO",),  # Added for VC mode
                "instruct_text": ("TEXT",),
            }
        }
    ...
    def generate(self, speed, inference_mode, sft_dropdown, seed, tts_text=None,
        ...
        elif inference_mode == '变声':
            print('get voice conversion inference request')
            print(self.model_dir)
            try:
                prompt_speech_16k = postprocess(speech)
                source_speech_16k = postprocess(source_speech)

                # 验证处理后的音频
                if prompt_speech_16k.numel() == 0 or source_speech_16k.numel() == 0:
                    raise ValueError("Processed audio is empty")

                set_all_random_seed(seed)
                output = list(self.cosyvoice.inference_vc(source_speech_16k, prompt_speech_16k))

                # 验证输出结果
                if not output:
                    raise RuntimeError("No output generated from voice conversion")
...
class LoadSRT:
    ...
    def INPUT_TYPES(s):
        files = [f for f in os.listdir(input_dir) if
                 os.path.isfile(os.path.join(input_dir, f)) and f.split('.')[-1] in ["srt", "txt"]]
        return {"required":
                    {"srt": (sorted(files),)},
                }
